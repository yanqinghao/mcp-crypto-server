...
async def _fetch_etf_single_series_data(
    ...
    try:
        ...
        etf_data = await fetch_etf_hist_data(
            ctx, symbol, market_type, "daily", start_date, end_date
        )
...
async def get_etf_candles(ctx: Context, inputs: ETFCandlesInput) -> CandlesOutput:
    ...
    Args:
        inputs.symbol: ETF代码 (如: 510050, 159919)
        inputs.timeframe: 时间框架 (daily, weekly, monthly)
        inputs.limit: 数据条数限制
        inputs.market_type: 市场类型 (A/HK/US)
...
async def get_etf_intraday_data(
    ...
    Args:
        symbol: ETF代码
        period: 时间周期 (1分钟/5分钟/15分钟/30分钟/60分钟)
        market_type: 市场类型
    """
...
async def search_etf_symbols(ctx: Context, inputs: ETFSearchInput) -> dict:
    ...
    Args:
        inputs.keyword: 搜索关键词 (如: 科技, 医疗, 510050)
        inputs.market_type: 市场类型
        inputs.limit: 结果数量限制
    """
    await ctx.info(
